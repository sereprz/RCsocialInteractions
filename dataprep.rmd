...
```{r findMentions, tidy = TRUE}
m <- regexpr('@\\*{2}(.*?)\\*{2}', messages$text, perl = TRUE)
mentions <- regmatches(messages$text, m)
network_df <- data.frame( messages[ grep('@\\*{2}(.*?)\\*{2}', messages$text), c('sender_id','sender_name')], mention_name = sub('\\*{2}', '', sub("@\\*{2}", '', mentions)) )
...
for(i in 1:length(ids)) {
    ...
    if(length(at) > 0) {
        ...
        network_m[rownames(network_m) == ids[i], colnames(network_m) == at[j]] <- network_m[rownames(network_m) == ids[i], colnames(network_m) == at[j]] + 1
        }
    }
}

write.table(network_m, 'network.csv', row.names = FALSE, col.names = FALSE, sep = ',')
```
#### Graphics
...
nodes <- list()
for(i in 1:(n_ids)) {
    nodes[[as.character(rownames(network_m)[i])]] <- list(id = i-1, sender_name = people$sender_name[ match(rownames(network_m)[i], people$sender_id) ], batch = 1, mentions_sent = as.numeric(mentions_sent[ rownames(network_m)[i]]), mentions_received = as.numeric(mentions_received[ rownames(network_m)[i]]) )
}
...
for(i in 1:nrow(network_m)) {
    if(any(network_m[i,] > 0)) {
        ...
        for(j in 1:length(l)) {
            links[[length(links)+1]] <- list(source = nodes[[rownames(network_m)[i]]]$id, target = nodes[[names(l)[j]]]$id, value = network_m[i, l[j]])
        }
    }
}
