...
```
#### Graphics
```{r igraph, tidy = TRUE, fig.width = 12, fig.height = 12}
require(igraph)
...
nodes <- list()
for(i in 1:(n_ids)) {
    nodes[[as.character(rownames(network_m)[i])]] <- list(id = i-1, sender_name = people$sender_name[ match(rownames(network_m)[i], people$sender_id) ], batch = 1)
}
